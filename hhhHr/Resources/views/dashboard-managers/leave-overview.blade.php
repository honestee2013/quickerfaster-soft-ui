<x-qf::livewire.bootstrap.layouts.dashboards.default-dashboard>
    <x-slot name="mainTitle">
        <strong class="text-info text-gradient">Leave Management Dashboard</strong>
    </x-slot>
    ...
    <div class="row g-4 mb-4">
            @foreach($widgetsConfig as $widgetId => $config)
                @if($config['type'] === 'balance-card')
                    <div class="{{ $config['size'] ?? 'col-12 col-sm-6 col-lg-3' }}">
                        <livewire:qf::widgets.cards.balance-card-widget
                            :widgetId="$widgetId"
                            :config="$config"
                            :key="'balance-'.$widgetId"
                        />
                    </div>
                @endif
            @endforeach
